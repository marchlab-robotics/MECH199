...
//========== PIN MAP ==========
#define PIN_SIG      A1        // sEMG sensor raw signal pin
...
void loop() {
  ...
  int pos = map(sEMG_SIG, -MAX_SIG/2, +MAX_SIG/2, -150, 150);   // map sEMG senser value(0~MAX_SIG) to servo motor angle (-150~150 degree)

  myservo.write(pos);   // move servo motor
  delay(1);                 // delay 1 ms for stablility

  // print for arduino serial plotter
  Serial.print(-MAX_SIG/2);        // print the lowest value
  Serial.print(",");      // print separator of valuse
  Serial.print(+MAX_SIG/2);  // print the highest value
  Serial.print(",");      // print separator of valuse
